    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $edit = '<a href="' . route('admin.testimonial.edit', $query->id) . '" class="btn btn-warning fas fa-edit "></a>';
                $delete = '<a href="' . route('admin.testimonial.destroy', $query->id) . '" class="btn btn-danger mx-2 delete-item fas fa-trash "></a>';
                return $edit . $delete;
            })
            ->addColumn('image', function ($query) {
                return "<img src='" . asset($query->image) . "'  style='width:60px;height:60px;' />";
            })
